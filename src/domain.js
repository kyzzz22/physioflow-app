...
export const step=(type='instruction',x={})=>{const d=STEP_DEFAULTS[type]||STEP_DEFAULTS.instruction;return{step_id:uid('step'),type,name:d.name,name_i18n:{zh:'',ja:'',en:''},role:'custom',appearance:{font_size:null,alignment:null,color:null,background:null,font_weight:null,padding:null,border:null,border_radius:null,custom_css:null},duration_mode:d.duration_mode,planned_duration_ms:d.planned_duration_ms,content:'',content_i18n:{zh:'',ja:'',en:''},stimulus_id:'',questionnaire_id:'',analysis_label:'',is_analysis_window:false,allow_skip:true,allow_retry:false,required:true,recovery_behavior:d.recovery_behavior||'resume_remaining',source_mode:MEDIA_TYPES.includes(type)?'url':'none',source_url:'',asset_id:'',file_name:'',start_mode:'auto',auto_advance:true,show_controls:true,muted:false,loop:false,volume:1,fixation_config:null,timer_config:null,response_config:null,rest_config:null,...defaultExtras(d),...x}};
export const trial=(x={})=>({trial_id:uid('trial'),name:'New trial',condition:'',repeat_count:1,iti_jitter_ms:0,iti_jitter_distribution:'uniform',layout:{background:'#fffef9',foreground:'#17221d',content_width:900,alignment:'center',padding:48,gap:24,show_progress:true,show_step_type:true,border_radius:12,custom_css:''},steps:[step()],...x});
...
export const protocol=(x={})=>({schema_version:'1.0.0',protocol_id:uid('protocol'),project_id:uid('project'),name:'Untitled experiment',version:1,version_name:'Draft 1',status:'draft',archived_at:null,created_at:new Date().toISOString(),updated_at:new Date().toISOString(),frozen_at:null,config_hash:null,app_version:APP_VERSION,theme:{primary_color:'#197453',primary_color_dark:'#5ed0a2',font_family:'system-ui',font_scale:1.0,border_radius:8,dark_mode_default:false,default_trial_background:'#fffef9',default_trial_foreground:'#17221d',default_trial_padding:48,default_trial_gap:24},blocks:[block()],stimuli:[],questionnaires:[],biodb:{experiment_id:'',experiment_label:''},...x});
...
export function stepContentIssues(step, stimuli, questionnaires) {
  ...
  if (step.type === 'custom_html' && !String(step.html || '').trim()) {
    issues.push({ kind: 'warn', key: 'empty_custom_html', message: '«Custom HTML» is empty — add HTML content or this step will show a blank screen' });
  }
  return issues;
}
