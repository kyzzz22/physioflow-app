...
export function NodeInspector({ node, definition, variables, groups, mode, onUpdate, onAssignGroup, onCreateGroup, onEditParticipantUi, questionnaireLibrary, onLibraryChange, assets, resources, stimulusPools = [], dataOutputOptions }) {
  ...
  return <div className="inspector-card">
    ...
    {mode !== 'quick' && node.config && <details className="advanced-fields"><summary>Analysis & recovery</summary>
      ...
      {node.config.deviceConnectorId && <label className="checkbox-row"><input type="checkbox" checked={node.config.deviceRequired !== false} onChange={event => onUpdate({ config: { ...node.config, deviceRequired: event.target.checked } })} /> Require this device before the session starts<small className="field-help">When enabled, connection or adapter errors block the run instead of silently collecting without device data.</small></label>}
    </details>}
