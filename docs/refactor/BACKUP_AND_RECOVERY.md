...
After restoration, provide a credential key capable of opening the snapshot and verifying its audit chain, start a server against the recovery paths, confirm `/readyz` returns HTTP 200, redeem a disposable test launch link if policy permits, and verify an exported Deployment package before promoting the recovery copy. Maintain multiple dated backups and rehearse restoration on a separate machine; backup creation alone is not recovery evidence. Backup inventory verification does not require the credential key, but usable recovery and audit verification do; keep keys in a separate protected recovery system as described in `CREDENTIAL_PROTECTION.md` and `AUDIT_INTEGRITY.md`.
