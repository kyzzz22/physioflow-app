...
- 已补（2026-08-26 P4 交互体验）：**拖拽连线**（数据输出端口按住拖至兼容输入端口释放即建边，临时线实时预览，Esc 取消且松开不误连接，点选连线保留）；**Ctrl+A 全选**（排除入口节点）、**Ctrl+=/Ctrl+- 缩放**（0.4×–2.5× 限幅）、**Ctrl+0 复位视图**；**Esc 分层取消语义**（拖拽连线 → 点选连线 → 框选 → 删除确认 → 清空选择），`App.jsx` 全局层不再拦截 Escape，编辑区内 Esc 永不退出 Builder。
